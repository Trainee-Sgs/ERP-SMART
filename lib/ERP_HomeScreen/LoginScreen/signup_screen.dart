...
class _SignupState extends State<SignupScreen> {
  ...
  bool _hide = true, _loading = false, _ok = false;
  double _strength = 0;
  String _sLabel = '', _industry = '';
  Color _sColor = const Color(0xFFEF4444);
  ...
  void initState() {
    super.initState();
    _pass.addListener(_checkStrength);
  }

  @override
  void dispose() {
    _pass.removeListener(_checkStrength);
    for (final c in [_first, _last, _phone, _company, _pass]) c.dispose();
    super.dispose();
  }
  ...
  void _checkStrength() {
    ...
    double s = 0;
    if (v.length >= 8) s += .25;
    if (RegExp(r'[A-Z]').hasMatch(v) && RegExp(r'[a-z]').hasMatch(v)) s += .25;
    if (RegExp(r'[0-9]').hasMatch(v)) s += .25;
    if (RegExp(r'[^A-Za-z0-9]').hasMatch(v)) s += .25;
    setState(() {
      _strength = v.isEmpty ? 0 : s;
      _sLabel = v.isEmpty
          ? ''
          : ['', 'Weak', 'Fair', 'Good', 'Strong'][(s * 4).ceil()];
      _sColor = s <= .25
  ...
  Future<void> _submit() async {
    if (!_form.currentState!.validate()) {
      ErpSnackbar.error(context, 'Please fill all required fields.');
      return;
    }
    ...
    ErpSnackbar.success(
      context,
      'Account created! Welcome to Smart ERP',
      title: 'All Done',
    );
  }
                      ...
                      _Card(
                        child: Column(
                          ...
                          children: [
                            ...
                            _Btn(
                              label: _ok ? 'ACCOUNT CREATED' : 'CREATE ACCOUNT',
                              loading: _loading,
                              ok: _ok,
                              onTap: _submit,
                            ),
  ...
  Widget build(BuildContext context) => Row(
    ...
    children: [
      Container(
        ...
        child: Icon(Icons.hub_rounded, color: Colors.white, size: 20.sp),
      ),
      SizedBox(width: 12.w),
      Text(
        'Smart ERP',
