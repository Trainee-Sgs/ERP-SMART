...
class HrmCustomButtom extends StatelessWidget {
  final AppModel app;
  final VoidCallback? onTap;

  const HrmCustomButtom({super.key, required this.app, this.onTap});
...
class HrmCustomBar extends StatelessWidget {
  final int selectedIndex;

  const HrmCustomBar({super.key, required this.selectedIndex});
  ...
  void _handleNavigation(BuildContext context, int index) {
    ...
    if (index == 0) {
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(builder: (context) => const HrmDashboardScreen()),
        (route) => false,
      );
    } else if (index == 1) {
      if (selectedIndex == 0) {
